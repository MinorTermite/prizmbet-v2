...
## 📊 Статус парсеров (v2.1 — март 2026)

| Парсер | Статус | Матчей | Примечание |
|--------|--------|--------|------------|
| **Leonbets** | ✅ | ~1090 | Работает стабильно, основной источник |
| **OddsAPI.io** | ✅ | ~41 | Free plan, rate limit 60 событий |
| **ApiFootball** | ✅ | ~30 | Free plan, топ-лиги Европы |
| **1xBet** | ⚠️ | 0 | Нужен прокси РФ (гео-блок) |
| **Pinnacle** | ⚠️ | 0 | Требуется аккаунт ps3838 |
| **the-odds-api.com** | ⚠️ | 0 | Требуется API ключ |

**Итого:** ~1160 матчей (без 1xBet и Pinnacle)

### Требуемые секреты (GitHub Secrets → Actions)

```env
# Обязательно для полного функционала
ODDS_API_KEY=sk_...              # the-odds-api.com → +200 матчей
PROXY_URL=socks5://...           # Прокси с РФ IP для 1xBet → +5000 матчей
PINNACLE_LOGIN=...               # ps3838 аккаунт
PINNACLE_PASSWORD=...
...
### Приоритет 1 (данные)
1. **Настроить ODDS_API_KEY** — +200 матчей из топ-лиг (5 минут)
2. **Настроить прокси для 1xBet** — +5000 матчей (30 минут)
3. **Исследовать Pinnacle API** — проверить актуальный эндпоинт

### Приоритет 2 (UX)
4. **Пагинация на frontend** — при 1000+ матчах
...
_Версия документа: 2.1 | Последнее обновление: 5 марта 2026_

_Статус парсеров: Leonbets ✅ (1090), OddsAPI.io ✅ (41), ApiFootball ✅ (30)_
